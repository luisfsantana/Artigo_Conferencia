...
\thanks{Luis Sant'Ana is with the Center Mathematics, Computation and Cognition, Federal University of ABC, Santo Andr\'{e}, SP, Brazil e-mail: luis.ana@ufabc.edu.br}% <-this % stops a space
...
We devise the fit curves using the well-known method of \textit{least squares}, with a
...
where $ f_i (x) $ corresponds to the following set of functions $x$, $x^2$, $x^3$,  $e^x$, $x$ and the combinations $xe^x$ and $xlnx$.
...
The pseudo-code for this phase is shown in Algorithm~\ref{algModel}. This code is executed
in a single node, called \emph{master node}. Variable $blockListSize$ contains the size
...
imbalances that will cause processor idlenesses. The threshold must be determined
empirically; values of about 10\% of the execution time of a single value
resulted in a good trade-off in practice.
...
\begin{figure}[!t]
	\centering
			\includegraphics[scale=0.25]{DiagramaArtigo.eps}
...
time of each task execution. If the difference between the finish times exceeds the define threshold,  the
algorithm devises new curves for each processor performance model and solve the
...
% DC: não entendi esse parágrafo. Quais valores foram obtidos com experimentos? O que significa um valor ser "melhor"?
% RYC: também não entendi. E a linguagem a lot of values were tesed não é apropriada. É preciso ser mais claro e específico. Talvez você possa descrever os parâmetros usados em cada uma das aplicações nas suas respectivas seções. 
...
\begin{figure*}[htb]
	...
	\centering
		...
		\includegraphics[scale=0.32]{1machine.pdf} \quad
		\includegraphics[scale=0.32]{2machines.pdf} \quad
		\includegraphics[scale=0.32]{3machines.pdf} \quad
		\includegraphics[scale=0.32]{4machines.pdf} 
...
Figure~\ref{fig:todosJuntos} shows the results for matrices with sizes varying from $4096 \times 4096$ to $65536 \times 65536$ elements, on the different scenarios (with different number of machines). We present the average execution time of 10 experiments, with a standard deviation of 4.87.
...
matrices with size $65535 \times 65535$.
Figure~\ref{fig:diferencaThreads}
shows the results for different number of threads for each algorithms. On the X axis are distributed threads of the same way among machines, including threads on the GPU and CPU. For two threads was used only the CPU for other cases CPU and GPU was used. In the y axis, we presented the maximum difference between the end of the performances of each algorithm for the same scenario. Our algorithm have the smallest amount of difference among all the algorithms implying almost perfect load balancing of processors. Other algorithms show considerable differences mostly because of the idle GPU at the end of execution. 
...
where incorrect predictions are maintained during the entire execution.
...
\begin{figure*}[htb]
	...
	\centering
		...
		\includegraphics[scale=0.32]{1machineBlack.pdf} \quad
		\includegraphics[scale=0.32]{2machineBlack.pdf} \quad
		\includegraphics[scale=0.32]{3machineBlack.pdf} \quad
		\includegraphics[scale=0.32]{4machineBlack.pdf} 
...
% RYC: Comentei por que esta explicação não me parecer boa. A limitação é mesmo
% a transferência de dados? Eu acho que não...
%
%To the environment with 3 and 4 machines the performance
%were similar due to limitation in data transfer, in the case of four machines
%the communication cost limited the gain. 
...
% RYC: Em cada gŕafico você está usando uma cor diferente para os algoritmos. É
% preciso ser consistente! -> OK

\begin{figure*}[htb]
	\begin{center}
	\centering
			%\includegraphics[scale=0.4]{GraficoFabrizio4maquinasNOVO.pdf}
		\includegraphics[scale=0.32]{1machineGenes.pdf} \quad
		\includegraphics[scale=0.32]{2machineGenes.pdf} \quad
		\includegraphics[scale=0.32]{3machineGenes.pdf} \quad
		\includegraphics[scale=0.32]{4machineGenes.pdf} 
...
% RYC: Quantas tarefas foram geradas? É uma tarefa por ponto? 
%Tentei fazer a análise independente das tarefas, apenas somei a quantidade de dados de cada tarefa.

% RYC: Aqui você colocou apenas os blocos das GPUs? E das CPUs?
% Eu dividi por máquina. Independe de GPU e CPU.

The topmost graph in Figure~\ref{fig:GeneBlocos} shows the block sizes generated
by our algorithm after the processor performance modeling phase. We can see
...
iteration, for all units complete execution at nearly the same time. This prevents that processors receive large portions data and other no, which causes some units to become idle. But even though there is this decrease in block size, the proportion
...
\IEEEtriggeratref{10}  % REAJUSTAR DEPOIS DE TODAS AS MODIFICACOES!
