...
% DC: corrigir as referências quebradas.
% DC: 3 ou 4 maquinas foram usadas? Ou foram 3 arquiteturas?
...
%DC: corrigir última linha da tabela
\begin{table*}[t]
\begin{scriptsize}
...
% DC: "$k$ blocks per with 1024 threads for each block" -> não entendi e não sei como arrumar a frase. :P
...
We have evaluated all four scheduling algorithms --- (1) our algorithm, (2) static, (3) HDSS, and (4) Greedy --- varying the number of machines.
...
greedy algorithm constantly presented the worst performance.
...
% DC: "There are big differences among end of threads." Quais?!
The greedy algorithm performed well, even considering that it does not directly
use information on the processing speeds of the devices. Actually, the algorithm uses this
information indirectly, since faster devices finish their tasks earlier and,
consequently, receive more tasks. The static algorithm had the worst performance because it keeps the thread idle for long periods of time, as we can see in Figure \ref{fig:diferencaThreads}. There are big differences among end of threads.
...
Our algorithm accurately estimate the amount of data that should be
...
We evaluate the execution time of the \emph{Blackscholes} application using different
...
gains were better with bigger problem sizes (number of options) and more
...
with this class of applications. Table~\ref{table: black} shows the extreme values ​​of 10,000 and 500,000 options, the best result was obtained with four machines, and with the largest number of options.
...
scenario with 4 machines, we also see that the overhead caused by solving the optimization
problem
%to determine the best data distribution
is small and the obtained gains
...
milliseconds, with standard deviation of 0.34 milliseconds
...
Table~\ref{table: gene} shows the extreme values​​, for 100,000 and 2,000,000 genes. 
...
Like the other experiments, the maximum performance difference among the threads followed the same principle. Our algorithm produced the smallest difference among the threads, as shown in Figure~\ref{fig:GeneDiferenca}.
...
% DC: the behavior do quê?
In Figure~\ref{fig:GeneBlocos} we compare the behavior in relation to
submission of blocks among machines. We analyzed the block size for the HDSS and
our algorithm. In the case where three machines were used, it is possible to notice
the difference between runs. During step 400, our algorithm triggered a re-balancing process,
